...
class AtmModel:
    ...
    def get_keys(pwv, elev):
        ...
        return [
            "%d,%d" % (int(round(pwv_ * m_to_mm, 1) * mm_to_um), int(round(elev_, 0)))
...
class Atmosphere(Model):
    ...
    def sample(self, nsamples):
        ...
        )  # pylint: disable=no-member  # 1e-3: PWV in mm -> meters for get_keys
...
class Foreground(Model):
    ...
    emiss = Variable(default=1.0, help="Emissivity")
...
class Dust(Foreground):
    ...
    def sample(self, nsamples):
        ...
        self.scale_temp = np.expand_dims(
