...
def check_inputs(x, inputs=None):
    ...
    if isinstance(x, np.ndarray) or isinstance(x, list):
        ...
        for inp in inputs:
            if callable(inp):
                ret.append(inp(x).astype(np.float64))
            elif isinstance(inp, np.ndarray) or isinstance(inp, list):
                ret.append(np.array(inp).astype(np.float64)*ones)
            elif isinstance(inp, int) or isinstance(inp, float):
                ret.append(np.array([inp for i in x]).astype(np.float64)*ones)
            else:
                raise Exception(
...
def spill_eff(freq, pixd, fnum, wf=3.0):
    ...
    return 1.0 - jnp.exp(
        (-jnp.power(np.pi, 2) / 2.0) * jnp.power((pixd / (wf * fnum * (c / freq))), 2)
    )
...
def ruze_eff(freq, sigma):
    ...
    return jnp.exp(-jnp.power(4 * np.pi * sigma / (c / freq), 2.0))
...
def ohmic_eff(freq, sigma):
    ...
    """
    # freq, sigma = _check_inputs(freq, [sigma])
    return 1.0 - 4.0 * jnp.sqrt(np.pi * freq * mu0 / sigma) / Z0
...
def inv_var(err):
    ...
    return 1.0 / (jnp.sqrt(jnp.sum(1.0 / (jnp.power(np.array(err), 2.0)))))
